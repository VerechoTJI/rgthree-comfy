...
def log_step(msg=None, status=None):
    ...
    if msg:
        tag = (
            f'{COLORS["YELLOW"]}[ Notice ]'
            if status == "Notice"
            else f'{COLORS["RESET"]}[Starting]'
        )
...
def build(without_tests=True, fix=False):
    ...
    ts_version_result = subprocess.run(
    ...
    ts_version = re.sub(
        r"^.*Version\s*([\d\.]+).*", "v\\1", ts_version_result.stdout, flags=re.DOTALL
    )
    ...
    for file in js_files:
        ...
        print(f"handling: {rel_path}")
        with open(file, "r", encoding="utf-8") as f:
            filedata = f.read()
        num = rel_path.count(os.sep)
        if rel_path.startswith("comfyui"):
            filedata = re.sub(
                r'(from\s+["\'])rgthree/', f'\\1{"../" * (num + 1)}rgthree/', filedata
            )
            filedata = re.sub(
                r'(from\s+["\'])scripts/', f'\\1{"../" * (num + 1)}scripts/', filedata
            )
        else:
